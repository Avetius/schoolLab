function LaboratoryWork() {
    ...
    this.tab_icons = function(id, class_name, class_img, img) {
        ...
        var icon_name1 = name.replace(" ","</br>")
        var icon_name = name.split(" ");
        $("#" + tab_shelf_id + "").append("<div class='tab_icons_group " + class_name + "'><div class='lab_icons_group'><img id='" + id + "' class='lab_inocs " + class_img + "' classimg='" +class_img+ "' src='assets/app/img/" + img + "' /></div><div class='lab_titles'>" + icon_name1 + "</div></div>");
        // img_icons_sorted();
    this.img_icons_sorted = function(){
        for (var i = 0; i <= 7; i++) {
            for (var j = 0; j < 5; j++) {
